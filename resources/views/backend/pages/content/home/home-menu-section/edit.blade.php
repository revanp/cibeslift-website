                ...
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    ...
                    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                        ...
                        <li class="breadcrumb-item active">
                            <a href="{{ url('admin-cms/content/home/home-menu-section/edit/'.$data['id']) }}" class="text-muted">Edit Menu Section</a>
                        </li>
                    </ul>
                </div>
            </div>
            ...
            <div class="card card-custom">
                <div class="card-header">
                    <div class="card-title">
                        <h3 class="card-label">Edit Menu Section</h3>
                    </div>
                ...
                <form action="{{ url('admin-cms/content/home/home-menu-section/edit/'.$data['id']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="row">
                                ...
                                <div class="tab-pane {{ $key == 'id' ? 'active' : '' }}" id="{{ $key }}Tab" role="tabpanel">
                                    <div class="row mt-5">
                                        <div class="form-group col-md-6">
                                            <label>Title</label>
                                            <input type="text" class="form-control" placeholder="Enter title" name="input[{{ $key }}][title]" value="{{ $data['home_menu_section'][$key]['title'] }}"/>
                                        </div>
                                        <div class="form-group col-md-6">
                                            <label>Call to Action</label>
                                            <input type="text" class="form-control" placeholder="Enter call to action" name="input[{{ $key }}][cta]" value="{{ $data['home_menu_section'][$key]['cta'] }}"/>
                                        </div>
                                        <div class="form-group col-md-12">
                                            <label>Description</label>
                                            <textarea name="input[{{ $key }}][description]" id="" rows="5" class="form-control">{{ $data['home_menu_section'][$key]['description'] }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
