                ...
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    ...
                    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                        ...
                        <li class="breadcrumb-item active">
                            <a href="{{ url('admin-cms/content/about-us/manufacture/edit/'.$data['id']) }}" class="text-muted">Edit</a>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    <div class="d-flex flex-column-fluid">
		<div class=" container ">
            <div class="card card-custom">
                ...
                <form action="{{ url('admin-cms/content/about-us/manufacture/edit/'.$data['id']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="products[]" multiple class="select2 form-control">
                                        @foreach ($products as $key => $val)
                                            <option value="{{ $val->id_product_id }}" {{ in_array($val->id_product_id, $data['manufacture_id_has_product_id']) ? 'selected' : '' }}>{{ $val->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                            ...
                                            <div class="col-12 col-form-label">
                                                <div class="checkbox-inline">
                                                    <label class="checkbox checkbox-success">
                                                        <input type="checkbox" name="is_coming_soon" {{ $data['is_coming_soon'] ? 'checked' : '' }}/>
                                                        <span></span>
                                                        Coming Soon?
                                                    </label>
                                                </div>
                                ...
                                <div class="tab-pane {{ $key == 'id' ? 'active' : '' }}" id="{{ $key }}Tab" role="tabpanel">
                                    <div class="row mt-5">
                                        <div class="form-group col-md-6">
                                            <label>Name</label>
                                            <input type="text" class="form-control" name="input[{{ $key }}][name]" placeholder="Enter name" value="{{ $data['manufacture'][$key]['name'] }}">
                                        </div>
                                        <div class="form-group col-md-6">
                                            <label>Description</label>
                                            <textarea name="input[{{ $key }}][description]" class="form-control" placeholder="Enter description">{{ $data['manufacture'][$key]['description'] }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
