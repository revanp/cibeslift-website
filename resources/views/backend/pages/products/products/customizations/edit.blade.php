                ...
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    ...
                    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                        ...
                        <li class="breadcrumb-item active">
                            <a href="{{ url('admin-cms/products/products/customizations/'.$id.'/edit/'.$data['id']) }}" class="text-muted">Edit Customization</a>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    <div class="d-flex flex-column-fluid">
		<div class=" container ">
            <div class="card card-custom">
                ...
                <form action="{{ url('admin-cms/products/products/customizations/'.$id.'/edit/'.$data['id']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                        @php
                                ...
                                <div class="tab-pane {{ $key == 'id' ? 'active' : '' }}" id="{{ $key }}Tab" role="tabpanel">
                                    <div class="row mt-5">
                                        <div class="form-group col-md-6">
                                            <label>Name</label>
                                            <input type="text" class="form-control @if($errors->has('input.'.$key.'.name')) is-invalid @endif" placeholder="Enter name" name="input[{{ $key }}][name]" value="{{ old('input.'.$key.'.name') ?? $data['product_customization'][$key]['name'] }}"/>
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <textarea name="input[{{ $key }}][description]" rows="5" class="form-control @if($errors->has('input.'.$key.'.description')) is-invalid @endif">{{ old('input.'.$key.'.description') ?? $data['product_customization'][$key]['description'] }}</textarea>
                        ...
                        @if (!empty($data['product_customization_feature_id']))
                            <div class="feature-box mt-5">
                                @foreach ($data['product_customization_feature_id'] as $k => $v)
                                    <input type="hidden" name="feature[{{ $k }}][id]" value="{{ $v['id'] }}">
                                            ...
                                            <div class="form-group picture_upload col-md-6">
                                                ...
                                                <div class="form-group__file">
                                                    <div class="file-wrapper">
                                                        <input type="file" name="feature[{{ $k }}][image]" class="file-input"/>
                                                        ...
                                                        <div class="tab-pane {{ $key == 'id' ? 'active' : '' }}" id="{{ $key }}FeatureTab" role="tabpanel">
                                                            <div class="row mt-5">
                                                                <div class="form-group col-md-12">
                                                                    <label>Name</label>
                                                                    <input type="text" class="form-control" placeholder="Enter name" name="feature[{{ $k }}][input][{{ $key }}][name]" value="{{ $v['product_customization_feature'][$key]['name'] }}"/>
                                                                </div>
                                                                <div class="form-group col-md-12">
                                                                    <label>Description</label>
                                                                    <textarea name="feature[{{ $k }}][input][{{ $key }}][description]" class="form-control">{{ $v['product_customization_feature'][$key]['description'] }}</textarea>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    @endforeach
                            ...
                            <div class="row justify-content-center">
                                <a href="#" class="btn btn-primary w-20 mt-5 add-item-feature" data-count="{{ count($data['product_customization_feature_id']) }}"><i class="flaticon2-plus"></i> Add Another Feature</a>
                            </div>
                        @else
                            <div class="feature-box mt-5">
                                <div class="feature-item">
