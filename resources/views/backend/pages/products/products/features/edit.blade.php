            ...
            <div class="d-flex align-items-center flex-wrap mr-1">
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    ...
                    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                        ...
                        <li class="breadcrumb-item">
                            <a href="{{ url('admin-cms/products/products/features/'.$id) }}" class="text-muted">USP</a>
                        </li>
                        <li class="breadcrumb-item active">
                            <a href="{{ url('admin-cms/products/products/features/'.$id.'/edit/'.$data['id']) }}" class="text-muted">Edit Feature</a>
                        </li>
                    </ul>
                </div>
            </div>
                ...
                <div class="card card-custom">
                    <div class="card-header">
                        ...
                        <div class="card-toolbar">
                            <a href="{{ url('admin-cms/products/products/features/'.$id) }}" class="btn btn-danger font-weight-bolder">
                    ...
                    <form action="{{ url('admin-cms/products/products/features/'.$id.'/edit/'.$data['id']) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="card-body">
                            <div class="row">
                                ...
                                @foreach ($lang as $key => $val)
                                    <div class="tab-pane {{ $key == 'id' ? 'active' : '' }}" id="{{ $key }}Tab" role="tabpanel">
                                        <div class="row mt-5">
                                            <div class="form-group col-md-6">
                                                <label>Name</label>
                                                <input type="text" class="form-control @if($errors->has('input.'.$key.'.name')) is-invalid @endif" placeholder="Enter name" name="input[{{ $key }}][name]" value="{{ !empty(old('input.'.$key.'.name')) ? old('input.'.$key.'.name') : $data['product_feature'][$key]['name'] }}"/>
                                            ...
                                            <div class="form-group col-md-6">
                                                ...
                                                <textarea name="input[{{ $key }}][description]" rows="5" class="form-control @if($errors->has('input.'.$key.'.description')) is-invalid @endif">{{ !empty(old('input.'.$key.'.description')) ? old('input.'.$key.'.description') : $data['product_feature'][$key]['description'] }}</textarea>
